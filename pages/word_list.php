...
?>
            ...
            <div class="word-card">
                ...
                <div style="text-align: right; margin-top: 10px;">
                    <?php if (isset($_SESSION['id_nguoi_dung'])): ?>
                        <button onclick="openSaveModal(<?php echo $id_tu_chuan; ?>, this)" class="btn <?php echo $da_thich ? 'btn-green' : 'btn-outline'; ?>">
                            <?php if ($da_thich): ?>
    ...
    <?php else: ?>
        <div class="empty-state"><i class="fas fa-search" style="font-size: 60px; color: #ddd; margin-bottom: 10px;"></i>
            <p style="color:#777;">Không tìm thấy từ vựng nào.</p>
        </div>
<?php endif;
}
?>
...
<body>
    ...
    <script>
        ...
        async function openSaveModal(id_tuvung, btnElement) {
            try {
                ...
                const {
                    ...
                    html: `<p style="margin-bottom:5px; text-align:left;">Chọn danh sách:</p>
                    ...
                    preConfirm: () => {
                        ...
                        return {
                            ...
                            // Lấy thêm tên danh sách để hiển thị lên nút
                            textName: select.value === 'new' ? input.value : select.options[select.selectedIndex].text
                        }
                    }
                });
                ...
                if (formValues) {
                    if (formValues.mode === 'new' && !formValues.val) {
                        Swal.fire('Lỗi', 'Tên danh sách trống', 'warning');
                        return;
                    }
                    ...
                    if (saveResult.status === 'saved') {
                        // CẬP NHẬT TRỰC TIẾP NÚT BẤM (KHÔNG LOAD LẠI TRANG)
                        btnElement.className = 'btn btn-green';
                        btnElement.innerHTML = `<i class="fas fa-check"></i> Đã lưu: ${formValues.textName}`;
                    ...
                    } else if (saveResult.status === 'removed') {
                        // TRƯỜNG HỢP BỎ LƯU
                        btnElement.className = 'btn btn-outline';
                        btnElement.innerHTML = `<i class="far fa-star"></i> Lưu từ`;
        ...
        function loadPage(pageNumber) {
            ...
            fetch(url).then(response => response.text()).then(html => {
                ...
                if (document.activeElement.id !== 'searchInput') window.scrollTo({
                    top: 0,
                    behavior: 'smooth'
                });
            });
